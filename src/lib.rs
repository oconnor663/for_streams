...
impl ToTokens for JoinMeMaybe {
    fn to_tokens(&self, tokens: &mut TokenStream2) {
        ...
        for (arm, name) in self.arms.iter().zip(&arm_names) {
            ...
            let body = &arm.body;
            initializers.extend(quote! {
                let mut #name = ::std::pin::pin!(::futures::future::maybe_done(
                    async #move_token {
                        // Note that `cancel` is exposed to the caller, so it's not a mixed-site
                        // identifier.
                        let cancel = || {
